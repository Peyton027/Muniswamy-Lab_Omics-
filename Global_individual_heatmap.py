...
OUTPUT_PDF = "Global_individual_heatmap.pdf"
...
COHORTS = [("Normal", 0, 8), ("Obese", 9, 17), ("Diabetic", 18, 24), ("HFrEF", 25, 34)]


def main():
    input_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(INPUT_CSV)
    if not input_path.exists():
        raise FileNotFoundError(f"Cannot find {input_path}. Put the CSV beside this script or pass a file path.")
    ...
    if missing_samples:
        raise ValueError("Missing required sample columns: " + ", ".join(missing_samples))

    # Verify the required target is still present.
    cleaned_symbols = df["gene_symbol"].astype(str).str.replace(" ", "", regex=False)
    tmem94_count = (cleaned_symbols == "TMEM94").sum()
    if tmem94_count != 1:
        raise ValueError(f"Expected exactly one TMEM94 row; found {tmem94_count}.")

    matrix = df[SAMPLE_ORDER].apply(pd.to_numeric, errors="coerce").to_numpy(dtype=float)
    if not np.isfinite(matrix).all():
        raise ValueError("All 35 sample columns must contain finite numeric heatmap values.")

    # Keep the supplied row order, so your CSV controls the gene order.
    row_labels = [
    ...
    fig.subplots_adjust(left=0.13, right=0.96, top=0.975, bottom=0.05)

    title_ax = fig.add_subplot(gs[0, :])
    title_ax.axis("off")
    title_ax.text(0.5, 0.70, "Person-by-Person Ion-Channel / Transporter Expression Heatmap",
                  ha="center", va="center", fontsize=16, fontweight="bold")
    title_ax.text(0.5, 0.20,
                  f"All retained genes (n = {n_genes}) • each value is centered to the 9-sample Normal-group mean",
    ...
    for row_index, label in enumerate(row_labels):
        labels_ax.text(0.99, row_index, label, ha="right", va="center", fontsize=font_size)

    strip_ax = fig.add_subplot(gs[1, 1])
    cohort_codes = np.array([[0] * 9 + [1] * 9 + [2] * 7 + [3] * 10])
    strip_ax.imshow(cohort_codes, aspect="auto", cmap=plt.get_cmap("tab10", 4), vmin=0, vmax=3)
    strip_ax.set_xlim(-0.5, len(SAMPLE_ORDER) - 0.5)
    strip_ax.set_xticks([])
    strip_ax.set_yticks([])
    for boundary in [9, 18, 25]:
        strip_ax.axvline(boundary - 0.5, color="black", linewidth=1.0)
    for spine in strip_ax.spines.values():
        spine.set_visible(False)
    for cohort, start, end in COHORTS:
        strip_ax.text((start + end) / 2, 1.55, cohort,
                      transform=strip_ax.get_xaxis_transform(),
                      ha="center", va="bottom", fontsize=9, fontweight="bold")

    heat_ax = fig.add_subplot(gs[2, 1])
    cmap = LinearSegmentedColormap.from_list("blue_white_red", HEATMAP_COLORS, N=256)
    norm = TwoSlopeNorm(vmin=-COLOR_LIMIT, vcenter=0, vmax=COLOR_LIMIT)
    image = heat_ax.imshow(matrix, aspect="auto", interpolation="nearest", cmap=cmap, norm=norm)
    heat_ax.set_xticks(np.arange(len(SAMPLE_ORDER)))
    heat_ax.set_xticklabels(SAMPLE_ORDER, rotation=90, fontsize=8)
    heat_ax.tick_params(axis="x", length=0, pad=3)
    heat_ax.set_yticks([])
    heat_ax.set_xlim(-0.5, len(SAMPLE_ORDER) - 0.5)
    heat_ax.set_ylim(n_genes - 0.5, -0.5)
    for boundary in [9, 18, 25]:
        heat_ax.axvline(boundary - 0.5, color="black", linewidth=1.0)

    cbar_ax = fig.add_subplot(gs[2, 2])
    cbar = fig.colorbar(image, cax=cbar_ax)
    cbar.set_label("Centered expression\n(log2-CPM deviation from the Normal mean)", fontsize=9)
