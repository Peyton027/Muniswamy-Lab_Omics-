...
OUTPUT_PNG = "magnesium_individual_heatmap.png"
OUTPUT_PDF = "magnesium_individual_heatmap.pdf"
...
def main():
    ...
    if df.empty:
        raise ValueError(
            "No genes were found with 'Magnesium channels' in the major_class column."
        )
    ...
    # Verify TMEM94 is present in the final magnesium-only panel.
    cleaned_symbols = (
        df["gene_symbol"]
        .astype(str)
    ...
    if tmem94_count != 1:
        raise ValueError(
            f"Expected exactly one TMEM94 row in the magnesium heatmap; found {tmem94_count}."
        )

    # Save an audit showing every magnesium gene included.
    audit_columns = [
        col for col in [
            "gene_id",
            "gene_symbol",
    ...
    df[audit_columns].to_csv(
        "magnesium_genes_included_in_heatmap.csv",
        index=False
    )
    ...
    title_ax.text(
        0.5,
        0.70,
        "Person-by-Person Magnesium Gene Expression Heatmap",
        ha="center",
        va="center",
        fontsize=16,
        fontweight="bold"
    ...
    title_ax.text(
        0.5,
        0.20,
        f"All retained magnesium-annotated genes (n = {n_genes}) • "
    ...
    print(f"Magnesium genes plotted: {n_genes}")
    print("TMEM94 validation: passed")
    print("Gene audit saved: magnesium_genes_included_in_heatmap.csv")
    print("Genes included:")
    print(", ".join(df["gene_symbol"].astype(str)))
