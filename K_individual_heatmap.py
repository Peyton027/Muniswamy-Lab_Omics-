...
OUTPUT_PDF = "potassium_individual_heatmap.pdf"
COLOR_LIMIT = 2.0
...
def main():
    ...
    if missing_samples:
        raise ValueError("Missing sample columns: " + ", ".join(missing_samples))

    # Keep only potassium-related genes.
    df = df[
    ...
    # Sort potassium genes by subcategory, then gene name.
    if "subcategory" in df.columns:
        df = df.sort_values(
            ["subcategory", "gene_symbol"],
            kind="stable"
    ...
    if not np.isfinite(matrix).all():
        raise ValueError("Sample columns contain missing or non-numeric values.")
    ...
    title_ax.text(
        0.5,
        0.70,
        "Person-by-Person Potassium Gene Expression Heatmap",
        ha="center",
        va="center",
        fontsize=16,
        fontweight="bold"
    ...
    title_ax.text(
        0.5,
        0.20,
        f"All retained potassium-annotated genes (n = {n_genes}) • "
